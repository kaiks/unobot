...
class UnoProxy
    ...
    if host? nick
      ...
      when /.*Top card: .*/
        ...
        if text.include? $bot.nick
          unless text.include? "#{$bot.nick} passes"
  ...
  def drawn_card(c)
    ...
    @ai_engine.drawn_card_action parsed[0] if parsed.length == 1
  ...
  def parse_hand(card_text, drawn = false)
    unless card_text.match('c')
      ...
      bot_debug "[parse_hand] parsed: #{cards}", 2
      if drawn
        @ai_engine.hand << cards
      else
        @ai_engine.hand = Hand.new(cards)
      end
      bot_debug "[parse_hand] parsed hand: #{@ai_engine.hand}"
      if @game_start_draw
  ...
  def update_game_state(text, card = nil)
    ...
    if action_text == 'draws' || action_text == 'passes.' || action_text == 'passes'
      ...
      @game_state.war! if card.is_offensive?

      @game_state.warwd! if @game_state.war? && card.special_card?
    end
  end
end
