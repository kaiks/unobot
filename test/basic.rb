...
class TestStrategy < Test::Unit::TestCase
  ...
  def test_skip
    ...
    assert_equal(path[2].map(&:to_s).to_s, %w[ys y6].to_s, "Wrong path: #{path_s}")
  end

  def test_wd4_skip
    @proxy.top_card = UnoCard.parse('b5')
    @bot.hand.from_text %w[g6 gs gs wd4]
    prepare_tracker

    path = @bot.calculate_best_path_by_probability_chain
    path_s = path[2].map(&:to_s).to_s
    assert_equal(path_s, %w[wd4g gs gs g6].to_s, "Wrong path: #{path_s}")
  end
  ...
  def test_skip_3
    @bot.hand.from_text %w[g6 g7 gs r9 wd4 y1 y7]
    prepare_tracker
    path = @bot.calculate_best_path_by_probability_chain
    path_s = path[2].map(&:to_s).to_s

    assert_equal(path_s, %w[g6 gs g7 y7 y1 wd4 r9].to_s,
                 "Wrong path: #{path_s}")
  end

  def test_wild
    @bot.hand.from_text %w[b6 r7 ww]
    prepare_tracker
    path = @bot.calculate_best_path_by_probability_chain

    assert_nil(path, "Path was supposed to be nil, was #{path} instead")
  end

  def test_wild_2
    @bot.hand.from_text %w[b6 bs ww]
    ...
    assert_equal(path_s, %w[wb bs b6].to_s,
                 "Wrong path: #{path_s}")
  end

  def test_wild_3
    @bot.hand.from_text %w[b6 b6 bs ww]
    prepare_tracker
    path = @bot.calculate_best_path_by_probability_chain

    path_s = path[2].map(&:to_s).to_s

    assert_equal(path_s, %w[wb bs b6 b6].to_s,
                 "Wrong path: #{path_s}")
  end

  def test_wild_4
    @bot.hand.from_text %w[bs gs bs ww rs r6 ys]
    prepare_tracker
    path = @bot.calculate_best_path_by_probability_chain
  ...
  def test_wild_5
    @bot.hand.from_text %w[bs rs bs wd4 rs r6 ys]
    prepare_tracker
    assert_not_nil(
  ...
  def test_wild_7
    puts 'Test wild 7'
    @bot.hand.from_text %w[wd4 gr gr]
    @proxy.top_card = UnoCard.new(:wild, :wild)
    prepare_tracker
    path = @bot.calculate_best_path_by_probability_chain
    path_s = path[2].map(&:to_s)
  ...
  def test_wild_8
    puts 'Test wild 8'
    @bot.hand.from_text %w[y0 y5 y8 ww gs rs ys]
  ...
  def test_wild_9
    puts 'Test wild 9'
    @bot.hand.from_text %w[wwd4 gs g6]
  ...
  def test_zero_1
    @bot.hand.from_text %w[b6 b0]

    @proxy.top_card = UnoCard.parse('wb')
    prepare_tracker
    path = @bot.calculate_best_path_by_probability_chain
    path_s = path[2].map(&:to_s).to_s
    assert_equal(path_s, %w[b0 b6].to_s,
                 "Wrong path: #{path_s}")
  end

  def test_turn_order
    @bot.hand.from_text %w[b9 bs y8 y9 ys]
    @proxy.top_card = UnoCard.parse('b8')
    prepare_tracker
    path = @bot.calculate_best_path_by_probability_chain
    path_s = path[2].map(&:to_s).to_s
    assert_equal(path_s, %w[y8 y9 ys bs b9].to_s,
                 "Wrong path: #{path_s}")
  end
